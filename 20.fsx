...
let rotate (tile: seq<seq<char>>) =
    tile |> Seq.transpose |> Seq.map Seq.rev

let flip (tile: seq<seq<char>>) = tile |> Seq.map Seq.rev

let noBorders<'a> (n: int): (seq<'a> -> seq<'a>) = Seq.skip 1 >> Seq.take (n - 2)
...
let parseTile (id: TileID) (tile: seq<seq<char>>): Tile =
    ...
    let left =
        Seq.map Seq.head tile |> Seq.fold folder 0

    let right =
        Seq.map Seq.last tile |> Seq.fold folder 0

    let bottom = Seq.last tile |> Seq.fold folder 0

    let n = Seq.length tile
    let value =
        seq {
            for rows in noBorders n tile do
                noBorders n rows
        }
...
let camera (tile: Tile) (tiles: list<Tile>) =
    ...
    let otherTiles =
        tiles |> List.filter (fun t -> t.ID <> tile.ID)
...
let rec cameraArray (connectingCameras: list<Camera>) (cameras: list<Camera>) =
    ...
    | head :: rest ->
        ...
        let l =
            if head.L.IsSome
               && List.exists (fun c -> c.ID = head.L.Value) cameras then
                [ (List.find
                    (fun c ->
                        c.ID = head.L.Value
                        && c.R.IsSome
        ...
        let r =
            if head.R.IsSome
               && List.exists (fun c -> c.ID = head.R.Value) cameras then
                [ (List.find
                    (fun c ->
                        c.ID = head.R.Value
                        && c.L.IsSome
        ...
        let t =
            if head.T.IsSome
               && List.exists (fun c -> c.ID = head.T.Value) cameras then
                [ (List.find
                    (fun c ->
                        c.ID = head.T.Value
                        && c.B.IsSome
        ...
        let b =
            if head.B.IsSome
               && List.exists (fun c -> c.ID = head.B.Value) cameras then
                [ (List.find
                    (fun c ->
                        c.ID = head.B.Value
                        && c.T.IsSome
        ...
        head
        :: (cameraArray more (List.filter (fun c -> c.ID <> head.ID) cameras))
...
module part1 =
    let solve (input: Map<TileID, list<string>>) =
        let tiles =
            ...
            |> Seq.collect
                (fun x ->
                    ...
                    let tile =
                        (x.Value |> Seq.map (fun x -> Seq.map id x))
        ...
        let cameras =
            tiles |> List.map (fun t -> camera t tiles)
